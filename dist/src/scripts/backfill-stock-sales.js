...
function buildDataSource() {
    ...
    return new typeorm_1.DataSource({ ...opts, entities: [], migrations: [], synchronize: false, logging: false });
}
function tracksInventory(candidate, globalTrackInventory) {
...
async function main() {
    ...
    const dbName = vendure_config_1.config.dbConnectionOptions.database;
    try {
        if (revertIndex !== -1) {
            await revert(dataSource, argv[revertIndex + 1]);
            return;
        }
        const rows = await dataSource.query(CANDIDATE_QUERY);
        ...
        const globalTrackInventory = (globalSettings === null || globalSettings === void 0 ? void 0 : globalSettings.trackInventory) === true;
        console.log(`\nDatabase: ${dbName}`);
        ...
        for (const c of actionable) {
            const before = (_a = runningStock.get(c.stockLevelId)) !== null && _a !== void 0 ? _a : c.stockOnHand;
            const after = before - c.quantity;
            runningStock.set(c.stockLevelId, after);
            planned.push({ candidate: c, before, after });
        }
        console.log(`${pad("ORDER", 18)}${pad("STATE", 17)}${pad("LINE", 6)}${pad("VARIANT", 9)}${pad("QTY", 5)}${pad("ON HAND", 9)}${pad("->", 4)}RESULT`);
        console.log("-".repeat(81));
        for (const p of planned) {
            const flag = p.after < 0 ? "  << NEGATIVE" : "";
            console.log(`${pad(p.candidate.orderCode, 18)}${pad(p.candidate.orderState, 17)}${pad(p.candidate.orderLineId, 6)}` +
        ...
        const receiptPath = receiptIndex !== -1
            ? argv[receiptIndex + 1]
        ...
        try {
            for (const p of planned) {
                ...
                const insertResult = await queryRunner.query(`INSERT INTO stock_movement
           ...
           RETURNING id`, [-c.quantity, c.productVariantId, c.orderLineId, c.stockLocationId]);
                const [inserted] = returnedRows(insertResult);
                if (!(inserted === null || inserted === void 0 ? void 0 : inserted.id)) {
                ...
                await queryRunner.query(`UPDATE stock_level SET "stockOnHand" = "stockOnHand" - $1, "updatedAt" = now() WHERE id = $2`, [c.quantity, c.stockLevelId]);
                entries.push({
        ...
        const receipt = { createdAt: new Date().toISOString(), database: dbName, entries };
...
async function revert(dataSource, receiptPath) {
    if (!receiptPath) {
    ...
    try {
        ...
        for (const entry of receipt.entries) {
            ...
            const deleted = returnedRows(await queryRunner.query(`DELETE FROM stock_movement WHERE id = $1 AND discriminator = 'Sale' RETURNING id`, [
            ...
            await queryRunner.query(`UPDATE stock_level SET "stockOnHand" = "stockOnHand" + $1, "updatedAt" = now() WHERE id = $2`, [entry.quantity, entry.stockLevelId]);
            reverted++;
        }
        await queryRunner.commitTransaction();
